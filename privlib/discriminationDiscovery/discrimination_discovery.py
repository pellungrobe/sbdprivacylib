...
class DD:
    def __init__(self, df, unprotectedDesc, negdecDesc, na_values=None, one_hot_set=None):
        if isinstance(df, pd.DataFrame):
            na_values = 'NaN' if na_values is None else na_values
            self.tDB, self.codes, self.decodes = PD2tranDB(df, na_values=na_values, one_hot_set=one_hot_set)
        else:
            na_values = '?' if na_values is None else na_values
            self.tDB, self.codes, self.decodes = CSV2tranDB(df, na_values=na_values, one_hot_set=one_hot_set)
        ...
        pos_decs = [self.codes[v] for v in self.codes if get_att(v)==self.decisionAtt and self.codes[v]!=self.neg_dec]
        if len(pos_decs) != 1:
    ...
    def print(self, ctg):
        ...
        spec = ('{:'+str(xlen)+'}|{:'+str(len(self.negdecDesc))+'}|{:'+str(len(self.posdecDesc))+'}|{:'+str(len(str(n)))+'}')
