...
class SequentialPrivacyFrame(pd.DataFrame):
    ...
    def __init__(self, data, user_id = constants.USER_ID, datetime = constants.DATETIME, order_id = constants.ORDER_ID,
                 sequence_id = constants.SEQUENCE_ID, elements = constants.ELEMENTS, timestamp = False, check_order_date=True):


        d_columns = {user_id : constants.USER_ID,
        ...
        if not isinstance(data, pd.core.internals.BlockManager):
            if isinstance(elements, str) or isinstance(elements, int):
            ...
            elif isinstance(elements, list) and (all(isinstance(x, str) for x in elements) or all(isinstance(x, int) for x in elements)):
                l=[]
                for i in elements:
                    l.append(spf[i])
                ...
                raise TypeError(f"PrivacyDataFrame constructor found elements of type: {type(elements)}")
            columns = spf.columns
        ...
        if not isinstance(data, pd.core.internals.BlockManager):
            ...
            if not self._is_SequentialPrivacyFrame():
                raise AttributeError("Some attributes where not secified at creation, columns are:" + str(self.columns) + 
                                     " but ''(%s, %s, %s, %s, %s)'' are needed" % (constants.USER_ID, constants.ORDER_ID, constants.SEQUENCE_ID, constants.DATETIME, constants.ELEMENTS) )
    ...
    def _check_order_date(self):
        ...
        if constants.DATETIME in self and constants.ORDER_ID in self:
            sort4sequence = self.sort_values(by=[constants.USER_ID, constants.SEQUENCE_ID, constants.ORDER_ID])
            sort4date = self.sort_values(by=[constants.USER_ID, constants.DATETIME])
            if not sort4sequence.equals(sort4date):
                raise AttributeError(f"PrivacyDataFrame {constants.DATETIME} attribute doesn't match with {constants.ORDER_ID}")

        if constants.DATETIME in self and constants.ORDER_ID not in self:
            self.sort_values(by=[constants.USER_ID, constants.SEQUENCE_ID, constants.DATETIME], inplace=True)
            self[constants.ORDER_ID]=0
            self._make_order()

        if constants.DATETIME not in self and constants.ORDER_ID not in self:
            self.sort_values(by=[constants.USER_ID,constants.SEQUENCE_ID], inplace=True)
            self[constants.ORDER_ID] = 0
            self._make_order()
            
        if constants.DATETIME in self and not pd.core.dtypes.common.is_datetime64_any_dtype(self[constants.DATETIME].dtype):
    ...
    def _make_order(self):
        ...
        for index, row in df_iterator:
            if (row[constants.USER_ID] == puid) and (row[constants.SEQUENCE_ID] == pseq):
    ...
    def _is_SequentialPrivacyFrame(self):
        return (constants.USER_ID in self) and (constants.ORDER_ID in self) and (constants.SEQUENCE_ID in self) and (constants.DATETIME in self)and (constants.ELEMENTS in self)


    @property
    def uid(self):
        if constants.USER_ID not in self:
            raise AttributeError("The PrivacyDataFrame does not contain the column '%s.'" % constants.USER_ID)
        return self[constants.USER_ID]
    
    @property
    def order(self):
        if constants.ORDER_ID not in self:
            raise AttributeError("The PrivacyDataFrame does not contain the column '%s.'" % constants.ORDER_ID)
        return self[constants.ORDER_ID]

    @property
    def datetime(self):
        if constants.DATETIME not in self:
            raise AttributeError("The PrivacyDataFrame does not contain the column '%s.'" % constants.DATETIME)
        return self[constants.DATETIME]

    @property
    def sequence(self):
        if constants.SEQUENCE_ID not in self:
            raise AttributeError("The PrivacyDataFrame does not contain the column '%s.'" % constants.SEQUENCE_ID)
        return self[constants.SEQUENCE_ID]

    @property
    def elements(self):
        if constants.ELEMENTS not in self:
            raise AttributeError("The PrivacyDataFrame does not contain the column '%s.'" % constants.ELEMENTS)
        return self[constants.ELEMENTS]

    @classmethod
    def from_file(cls, filename, user_id = constants.USER_ID, datetime = constants.DATETIME, order_id = constants.ORDER_ID,
                 sequence_id = constants.SEQUENCE_ID, elements = constants.ELEMENTS, timestamp = False, check_order_date=True, 
                  column_type=None, encoding=None, usecols=None, header='infer', sep=","):

        df = pd.read_csv(filename, sep=sep, header=header, usecols=usecols, encoding=encoding)

        return cls(df, user_id = user_id, datetime = datetime, order_id = order_id,
                 sequence_id = sequence_id, elements = elements, timestamp = timestamp, check_order_date=check_order_date)
